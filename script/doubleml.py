...
flux=pd.read_csv("data/res_grow_wd_pFBA/exchanges.csv",index_col=0)
selected=pd.read_csv("data/res_grow_wd_pFBA/selected_sample.csv")
flux=flux[flux.sample_id.isin(selected.x)]
flux=flux[flux["metabolite"].str.endswith("_m")]
flux=flux.iloc[:,[0,3,5]]
flux=flux.pivot_table(index="sample_id",columns="metabolite",values="flux").fillna(0)
...
flux=flux.iloc[:,flux.columns.str.endswith("_m")]
index=list()
for i in range(flux.shape[1]):
  tmp=np.mean(flux.iloc[:,i]!=0)
  if tmp>0.1 and len(flux.iloc[:,i].unique())>3:
    index.append(i)
flux = flux.drop(index="SZAXPI015258_57")
meta=meta[meta[""]!="SZAXPI015258_57"]
flux=flux.iloc[:,index]
scaler = StandardScaler()
scale_flux = scaler.fit_transform(flux)
...
np.random.seed(42)
df = pd.DataFrame(scale_flux, columns=flux.columns,index=flux.index)
meta.index=meta["sample_id"]
df["group"]=meta["study_condition"].map({"control" : 0,"pre-hypertension":1,"hypertension":2})
df["age"]=pd.Categorical(meta["age_category"],categories=["adult","senior"],ordered=True)
df["country"]=pd.Categorical(meta["country"],ordered=False)
...
for i in range(len(df.columns)-3):
  d=str(df.columns[i])
  df1=df.loc[:,[d,"group","age","country"]]
  df1=pd.get_dummies(df1,columns=["age","country"])
  ...
  pio.write_image(fig, "output/sensitivity/"+d+"_sensitivity.png", scale=3)

res.index=df.columns[:-3]
res.to_csv("output/doubleml.csv")
...
res.to_csv("output/doubleml_diastolic.csv")
...
res.index=df.columns[:-19]
res.to_csv("output/doubleml_systolic.csv")
