...
tmp<-grid.arrange(test_alpha_p,lefse_bar,ncol=1,heights=c(0.35,0.65))
dev.off()
pdf("output/fig1.pdf",width = 12,height = 7)
grid.arrange(tmp,test_beta_p,ncol=2,widths=c(0.6,0.5))
dev.off()
...
rel_ab<-test_mpse@assays@data@listData[["RelRareAbundanceBySample"]]
...
flux<-flux[meta$Sample.Name,]
flux[is.na(flux)]<-0
flux<-flux[meta$Sample.Name,]
flux[flux!=0]<-1

ab<-read.csv("data/test_cohort/test_16s/species_16s.csv",row.names = 1)
#ab_sample<-as.character(read.csv("data/abundance_short.csv",row.names = 1,header = F)[1,])
#colnames(ab)<-ab_sample
ab<-ab[,meta$Sample.Name]
ab[ab!=0]<-1
...
ab<-as.data.frame(ab)
meta_test<-meta_test[meta_test$Sample.Name%in%selected$x,]
meta<-data.frame(row.names = c(meta$Sample.Name),sample=c(meta$Sample.Name),group=meta_test$group)
meta<-meta[order(meta$group),]
ab_all<-ab[,meta$sample]
flux_all<-flux[,meta$sample]
flux_all<-flux_all[rowSums(flux_all)>50,]
data_all<-t(rbind(flux_all,ab_all))

row_ha = rowAnnotation(disease = meta$group,col = list(disease = c("healthy"="#4f6db7","hypertension"="orangered4")),
...
p_heat_all<-Heatmap(data_all, name = "Existence", cluster_rows = F,cluster_columns = F,
                    ...
                    row_split = factor(rep(c(" ", "  "), c(sum(meta$group=="hypertension"),sum(meta$group=="healthy"))),levels = c("  "," ")),
